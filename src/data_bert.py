...
def propagation_operators(args, edgelist):
    user_id_map = {user_id: idx for idx, (user_id, _) in enumerate({edge[0] for edge in edgelist})}
    item_id_map = {item_id: idx for idx, (_, item_id) in enumerate({edge[1] for edge in edgelist})}
