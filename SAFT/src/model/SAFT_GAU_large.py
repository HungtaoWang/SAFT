...
class SAFTEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.silu = nn.SiLU()
        self.to_out = nn.Sequential(nn.Linear(config.hidden_size, config.hidden_size), nn.Dropout(config.dropout))
    ...
    def propagation(self, token_encodings, propogation_matrix):
        ...
        gate = self.hidden1(encoder_outputs)
        gate = self.silu(gate)
        v = self.hidden2(encoder_outputs)
        v = self.silu(v)
        encoder_outputs = v
        token_encodings = v
        for j in range(self.prop_layers):
            ...
            out = gate * temp
            token_encodings = self.to_out(out)
            token_encodings = F.normalize(token_encodings, p=2, dim=1)
    ...
    def forward(self, token_edges_batch, hidden_states, attention_mask, query_embedding, key_embedding, user_operator, item_operator, distance_embedding, centrality_embedding):
        ...
        item_operator = self.sparse_prop(item_operator).to(user_operator.device)
